...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <h1>@lang('site.Trashed_groups')</h1>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}"> @lang('site.Home')</a></li>
                        <li class="breadcrumb-item active"> @lang('site.Trashed_groups')</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="row mb-3">
                        <div class="col-12 d-flex justify-content-between">
                            <a href="{{ route('group.create') }}" class="btn btn-success header-btn ">@lang('site.Add')
                                @lang('site.group')</a>
                            <a href="{{ route('group.index') }}" class="btn btn-warning header-btn ">@lang('site.All')
                                @lang('site.groups')
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div id="table-data" class="table-responsive">
                                @include('dashboard-views.group.pagination_data', ['pageType' => 'trashed'])
                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        languages['permanent_delete_group_title'] = "@lang('site.Delete') @lang('site.the_group')";
        languages['permanent_delete_group_body'] =
            "@lang('site.confirm') @lang('site.delete') @lang('site.the_group') " +
            "{{ $currentLanguage == 'ar' ? '؟' : '?' }}";
        languages['permanent_delete_group_url'] = "{{ route('group.permanent_delete') }}";
        languages['permanent_delete_group_action_btn_text'] = "@lang('site.Permanent_delete')";


        languages['restore_group_title'] = "@lang('site.Restore') @lang('site.the_group')";
        languages['restore_group_body'] = "@lang('site.confirm') @lang('site.restore') @lang('site.the_group') " +
            "{{ $currentLanguage == 'ar' ? '؟' : '?' }}";
        languages['restore_group_url'] = "{{ route('group.restore') }}";
        languages['restore_group_action_btn_text'] = "@lang('site.Restore')";
        // End defining languages

        // Start include pagination script
        const fetchDataURL =
            "{{ route('group.pagination.fetch_data') }}", // This valriable used in pagination_script
            pageType = 'trashed';
        ...
        $(document).ready(function() {
            ...
            $('#confirm_modal').on('show.bs.modal', function(event) {
                ...
                const groupId = button.data('group_id');
                ...
                $('#confirm_form').attr('action', languages[`${confirmModalType}_group_url`]);

                // Change modal title
                $('#modal-title').text(languages[`${confirmModalType}_group_title`]);

                // Change modal body
                $('#modal-body p').text(languages[`${confirmModalType}_group_body`]);

                // Change modal action button text
                $('#action-btn-text').text(languages[`${confirmModalType}_group_action_btn_text`]);

                // Set input with button data-group_id
                $('.modal #group_id').val(groupId);
            });
                        ...
                        if (dataType == 'restore') {
                            if (response.status == 1) {
                                toastr.success(
                                    ...
                                    $(
                                        `a[data-group_id="${basicData.group_id}"]`
